...
function enhanceText(text) {
  ...
  result = result.replace(
    /([A-Z]{2,})(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)/gi,
    '$1 $2'
  );
...
function extractCandidateDetailsFromFileName(fileName) {
  ...
  if (parts.length >= 3) {
    ...
    const lastNameParts = parts
      .slice(1, parts.length - 1)
      .filter((part) => part.toLowerCase() !== 'resume');
...
function parseSkills(skillsText) {
  ...
  const cleanedText = cleanText.replace(
    /(Hard Skills:|Programming Languages:|Tools and Software:|don’t forget.*)/gi,
    ''
  );
  ...
  while (i < lines.length) {
    ...
    if (duration) {
      ...
      if (
        !role &&
        i < lines.length &&
        splitHeaderLine(lines[i]).duration === ''
      ) {
        // no extra skip
      } else {
...
function cleanPage(
  ...
  const headerCandidates = new Set(
    extractHeaderFooterCandidates(lines, numLines).filter(
      (line) => frequency[line] >= threshold
    )
  );
...
function mapToApplicant(rawData, details = {}, filePath) {
  ...
  if (details.applicantId) {
    rec.document_id = details.applicantId;
  }
...
export default {
  parseResumePdf,
  parseZipResumes,
  extractCandidateDetailsFromFileName,
  mapToApplicant
};
